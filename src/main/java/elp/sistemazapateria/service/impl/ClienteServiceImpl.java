...
import elp.sistemazapateria.service.mapper.ClienteMapper;
	 ...
	 @Autowired
	   ...
	   /* @Override
	    public void saveCliente(ClienteRequest request) {
	        ...
	        //clienteNew.setVentas(new ArrayList<>());
...
*/
	    @Override
	    public void saveCliente(ClienteRequest request) {
	        // Eliminar mapeo manual y usar el mapper
	        Cliente clienteNew = clienteMapper.toCliente(request);
	        // System.out.println("que llega" + clienteNew); // Eliminar
	        clienteRepository.save(clienteNew);
	    }

	    // ClienteServiceImpl.java - Corrección en updateCliente
	    @Override
	    public void updateCliente(Long id, ClienteRequest request) {
	        Cliente cliente = clienteRepository.findById(id).orElse(null);
	        if (cliente != null) {
